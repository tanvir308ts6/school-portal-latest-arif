...
import { EducationalLevelFormComponent } from './educational-level-form/educational-level-form.component';
...
export class EducationalLevelComponent implements OnInit {
  ...
  tableStructure: any = {
    headers: ['id', 'title', 'institute.name_en', 'status', 'actions'],
    banned: ['Sl No', 'Title', 'Institute', 'Status', 'Actions'],
  };
  tableButtons: any[] = [
    {
      tooltip: 'Edit',
  ...
  actionForAdd() {
    const dialogRef = this.dialog.open(EducationalLevelFormComponent, {
  ...
  addData(data: any, dialogRef: any) {
    this.api.post('settings/educational-level', data).subscribe(
      (response) => {
        this.reloadTable();
  ...
  editData(data: any, id: number, dialogRef: any) {
    this.api.update(`settings/educational-level/${id}`, data).subscribe(
      (response) => {
        this.reloadTable();
        this.showBackendMessage(response.response);
        this.loaderService.setLoaderStatus(false);
  ...
  deleteInventoryCategory(id: number, dialogRef: any) {
    this.api.delete('settings/educational-level/', id).subscribe(
      (response) => {
        this.reloadTable();
