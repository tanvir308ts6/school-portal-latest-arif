  ...
  implements OnInit
{
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<PostOfficeFormComponent>,
    ...
    this.formItemsData = this.formBuild.group({
      ...
      division: [],
      post_office_code: ['', checkRequired('Post Code')],
  ...
  getThanaDropdown(has_event?: any) {
    ...
    if (this.formItemsData.value.district_code) {
      ...
      this.api
        .get(`settings/thana?search=${this.api.getSearchData(tempSearch)}`)
        .subscribe((response: any) => {
  ...
  setDataForEdit(sourceData?: boolean) {
    if (this.data.data) {
      this.formItemsData.patchValue({
        ...
        post_office_code: this.data?.data?.post_office_code,
  ...
  // setAutocompleteData(fromControl: string, toControl: string, key: string) {
  //   let tempObj: any = this.formItemsData.value;
  //   if (tempObj[fromControl] && typeof tempObj[fromControl] === 'object' && tempObj[fromControl][key]) {
  //     this.formItemsData.controls[toControl].setValue(
  //       this.formItemsData.value[fromControl][key]
  //     );
  //   }
  // }
  // startValidationData(checkControl: string, targetControl: string){
  //   if(!this.formItemsData.value[checkControl]){
  //     this.formItemsData.controls[targetControl].setValue(null);
  //   }
  // }
}
