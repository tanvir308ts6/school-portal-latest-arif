...
export class CountryComponent implements OnInit {
  ...
  tableStructure: any = {
    headers: ['id', 'en_name', 'bn_name', 'country_code', 'actions'],
    banned: [
      'Sl No',
      'Name(English)',
      'Name(Bangla)',
  ...
  addData(data: any, dialogRef: any) {
    this.api.post('settings/country', data).subscribe(
      (response) => {
        this.reloadTable();
  ...
  editData(data: any, id: number, dialogRef: any) {
    this.api.update(`settings/country/${id}`, data).subscribe(
      (response) => {
        this.reloadTable();
        this.showBackendMessage(response.response);
        this.loaderService.setLoaderStatus(false);
  ...
  deleteInventoryCategory(id: number, dialogRef: any) {
    this.api.delete('settings/country/', id).subscribe(
      (response) => {
        this.reloadTable();
