...
@Component({
  selector: 'app-promote-student-form',
  templateUrl: './promote-student-form.component.html',
  styleUrls: ['./promote-student-form.component.scss'],
})
export class PromoteStudentFormComponent implements OnInit {
  config:
    | {
        timeOut: number;
        closeButton: boolean;
  ...
  displayedColumns: string[] = ['checkbox','id', 'name', 'roll'];
  class_id: any = '';
  size = 10;
  page = 1;
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<PromoteStudentFormComponent>,
  ...
  ngOnInit(): void {
    console.log(this.data.session_id);
    // this.formItemsData.controls['session_id'].setValue(this.data.sessionObj.id);
    // this.formItemsData.controls['class_id'].setValue(this.data.class_id);
    this.promotionStat = this.data.promotionStat;
    if(this.promotionStat=='Promotion'){
      this.btnForPromotOrNot = 'Promote';
      this.title = 'Promote'
    }
    if(this.promotionStat=='Not Promoted'){
      this.btnForPromotOrNot = 'Not Promoted';
      this.title = 'Not Promote'
      this.formItemsData.controls['class_id'].setValue(this.data.classObj.id);
    
    }
    this.sessionDropdown = this.data.dropdowns.session;
    console.log('this.sessionDropdown',this.sessionDropdown);
  ...
  validateForm(data?: any) {
    ...
    if (this.formItemsData.valid) {
        ...
        newArr = resultArr.map(
          ({ id, section_id, class_id, class_roll, group_id,class_shift_id,session_id }) => {
            ...
            return {
              ...
              session_id:this.formItemsData.value.session_id,
      ...
      if (this.promotionStat == 'Promotion') {
        this.enrollment_process = 'Promotion';
      
      }
      if (this.promotionStat == 'Not Promoted') {
        this.enrollment_process = 'Not Promoted';
